...
export default {
  ...
  actions: {
    ...
    async login ({ dispatch }, token) {
      ...
      const res = await user.getDataByToken();
      // 设置 cookie 一定要存 uuid 和 token 两个 cookie
      // 整个系统依赖这两个数据进行校验和存储
      // uuid 是用户身份唯一标识 用户注册的时候确定 并且不可改变 不可重复
      // token 代表用户当前登录状态 建议在网络请求中携带 token
      // 如有必要 token 需要定时更新，默认保存一天
      let {username, nick} = res.data.data;
      util.cookies.set('uuid', jwt.getClaim("uuid"));
      util.cookies.set('token', token)
      // 设置 vuex 用户信息
      await dispatch('d2admin/user/set', { name: nick ? nick : username}, { root: true })
      // 用户登录后从持久化数据加载一系列的设置
      await dispatch('load')
    },
    /**
    ...
    logout ({ commit, dispatch }, { confirm = false } = {}) {
      ...
      async function logout () {
        ...
        // 跳转路由
        location.href = "http://example.kyu92.top/login";
      }
      // 判断是否需要确认
      if (confirm) {
